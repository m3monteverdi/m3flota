...
async function openOT(id) {
 ...
 async function enviarAlertasEmail() {
   ...
   vencs.forEach(function(v){
     htmlBody += '<li><strong>'+v.cam+'</strong> - '+v.tipo+': '+v.fecha+' ('+v.dias+' dÃ­as)</li>';
   });
   htmlBody += '</ul>';
   try {
...
async function importGPS(input) {
  ...
  try {
    ...
    for (var s=0; s<wb.SheetNames.length; s++) {
      ...
      if (!camionId) {
        pestaÃ±asIgnoradas.push(sheetName);
        continue;
      }
      pestaÃ±asEncontradas.push(sheetName+' â†’ '+camionId);
    ...
    var resumen = 'PestaÃ±as: '+wb.SheetNames.length+' | Procesadas: '+pestaÃ±asEncontradas.length;
    if (pestaÃ±asIgnoradas.length) resumen += ' | IGNORADAS: '+pestaÃ±asIgnoradas.join(', ');
